...
Lower AI bills, without changing how you work. It quietly steps in before you blow your budget, trims runaway output, and sends image reads to a cheaper subagent — running in the background so you don't have to think about it.
...
Everything is controlled by `~/.claude/cost-opt.json`, written by the AMBridge PWA Save Money page. **All three features are off by default.** A missing key, a `null` value, or an `enabled` set to anything other than `true` is treated as disabled. Only an explicit `"enabled": true` activates a feature.
...
| Feature | Default | How to enable | Key defaults |
|---|---|---|---|
