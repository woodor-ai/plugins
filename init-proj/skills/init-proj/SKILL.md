...
1. **检测 amp 是否安装**（第一步，未检测到即停）：在 `~/AIAgent/AMBridge` 目录下跑：

   ```
   python3 -c "import amp" 2>/dev/null
   ```

   退出码非 0 视为未安装。若**未安装**，主 agent 不得继续采集参数、不得调 CLI，直接把以下英文原样输出给用户，然后结束 skill：

   ```
   amp is not installed on this machine. This is a paid feature. Please visit https://woodor.ai/plugins for details and to download and install it. Thank you.
   ```
