...
1. Run `~/.claude/meeting/bin/room candidates` to get session-name candidates. Output is TSV: `<status>\t<name>\t<info>` where status is one of:
   - `online` — registered AND monitor pid alive → choosing this would conflict with the running session, **skip from picker**.
...
2. Use the `AskUserQuestion` tool to let user pick. Build options from candidates:
   - All `stale` and `historical` names become options (label = `<name>` with description = tier + info).
   - "Other" is added automatically by AskUserQuestion for typing a brand-new name.
   - Skip `online` names — those are owned by other live sessions.
   - Cap at 4 options total (AskUserQuestion limit); if more than 3 stale+historical candidates, prefer the most recent / most-used.
3. After user picks a name (or types one via Other), proceed to "On `/meeting <name>`" below with that name.
