...
def test_normal_messages_are_coalesced_in_global_id_order():
    ...
    assert text == (
        "📬 New Message from alice@one to plugins@tools "
        "[via woodor:agent-meeting] Message ID: 101\n"
        "📬 New Message from bob@two to plugins@tools "
        "[via woodor:agent-meeting] Message ID: 104"
    )
...
def test_global_identity_and_sender_keep_canonical_star_suffix():
    ...
    assert text == (
        "📬 New Message from global-peer@* to global-agent@* "
        "[via woodor:agent-meeting] Message ID: 1"
    )
