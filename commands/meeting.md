...
- **Empty** (`/meeting`): Run `~/.claude/meeting/bin/room candidates`, parse it, and use the `AskUserQuestion` tool to let the user pick a name. Include ALL candidates (online / stale / historical), labeling online ones as `(in use — will conflict)`. After user picks, treat the choice as the name and proceed with registration per the meeting skill's "On `/meeting <name>`" steps.

- **`list` or `rooms`**: Run `~/.claude/meeting/bin/room list` and print the output verbatim. No registration.

- **`candidates`**: Run `~/.claude/meeting/bin/room candidates` and print the output verbatim (status, name, info).
...
- **Anything else (a name)**: Treat as a session name. Run the meeting skill's full registration flow:
  ...
  3. Atomic write into `~/.claude/meeting/directory.json` via `jq → tmp → mv`
  4. Run `~/.claude/meeting/bin/room init` (idempotent)
